            ...
            <span class="text-blue-400 d-flex align-items-center pb-1">
                <svg fill="#29b6f6" class="mr-1" xmlns="http://www.w3.org/2000/svg" width="14" height="14"
                    viewBox="0 0 512 512"><path
                        ...
                        {{ date('jS M, Y',strtotime($val->remind_from))}}
            </span>
            <h6 class="m-0 font-weight-semibold text-grey-400">{{ $val->title}}</h6>
            @if($val->priority == 0)
                <p class="m-0 text-muted"><span class="badge bg-blue badge-pill mb-1">Low</span></p>
            @elseif($val->priority == 1)
                <p class="m-0 text-muted"><span class="badge bg-warning badge-pill mb-1">Medium</span></p>
            @elseif($val->priority == 2)
                <p class="m-0 text-muted"><span class="badge bg-red badge-pill mb-1">High</span></p>
            @endif
        </li>
    @endforeach
@else
